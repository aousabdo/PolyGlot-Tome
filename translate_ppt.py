...
# Import your existing translation functions and configurations from book_translator.py
...
async def translate_text(text: str, source_lang: str, target_lang: str, model: str, retries: int = 3) -> str:
    ...
    return await translate_chunk(None, text, source_lang, target_lang, model, retries)

# New function to process text frames
async def process_text_frame(text_frame, source_lang, target_lang, model, retries):
    for paragraph in text_frame.paragraphs:
        # Adjust alignment and direction based on target language
        if target_lang in ['Arabic', 'Hebrew']:
            paragraph.rtl = True
